...
/// </summary>
public interface IBlockEncoder<TKey>
{
    /// <summary>
    /// Encodes the given entries into a <see cref="Block"/>. The keys are read from <paramref name="encodedKeys"/>
    /// using the offsets and lengths carried by each <see cref="BlockEntry{ValueBuffer}"/>, so they are not re-encoded.
    /// </summary>
    ...
    Block<TKey> Encode(ReadOnlyMemory<byte> encodedKeys, IReadOnlyList<BlockEntry> entries);
    ...
    /// <returns></returns>
    Block<TKey> Decode(ReadOnlyMemory<byte> buffer);
    ...
    Block<TKey> Decode(IMemoryOwner<byte> owner, int length);
    ...
    /// </summary>
    int EstimateSize(int encodedKeyLength, ValueBuffer value);
}
